    ...
    try {
        ...
        mappings.forEach(m => {
            ...
            options.forEach(opt => {
                ...
                const src = imgEl.src;
                if (!src) {
                    console.log([MISSING MAPPING]  ->  has no image mapped!);
                    missingCount++;
                } else {
                    // Extract just the filename part from file:///...
                    const filename = src.split('images/')[1];
                    if (!filename) {
                         console.log([INVALID SRC]  ->  gave src: );
                         missingCount++;
                         return;
                    }
                    ...
                    if (!fs.existsSync(diskPath)) {
                        console.log([FILE NOT FOUND]  ->  mapped to , but it doesn't exist on disk!);
                        missingCount++;
                    }
                }
            });
        ...
        console.log(\nVerification Complete.  options checked.);
        if (missingCount === 0) {
            console.log("SUCCESS: All options have valid image mappings that exist on disk!");
        } else {
            console.log(FAILURE:  options failed verification.);
        }
    } catch(e) {
